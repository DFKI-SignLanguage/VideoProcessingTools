...
parser = argparse.ArgumentParser(description="Uses mediapipe to extract the face mesh data from the frames of a video.")
...
parser.add_argument('--no-head-movement',
                    help='TODO -- If specified, neutralizes the head movement,'
                         ' i.e., the face mandmarks are saved without translation and rotation,'
...
# Extract the facedata from the specified videofile
def extract_face_data(filename: str) -> np.ndarray:
    ...
    if not cap.isOpened():
        raise Exception("Couldn't open video file '{}'".format(filename))
...
if __name__ == '__main__':
    ...
    facedata = extract_face_data(filename=video_filename)
